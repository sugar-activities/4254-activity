...
class CombineMerger(model_allele.Allele):
    """CombineMerger
    inv: cairo.OPERATOR_CLEAR <= self.left_operator <= cairo.OPERATOR_SATURATE
    inv: cairo.OPERATOR_CLEAR <= self.right_operator <= cairo.OPERATOR_SATURATE
    """
...
# not usable                             ('saturate',  cairo.OPERATOR_SATURATE),
    ...
    def __init__(self, trunk):
        ...
        super(CombineMerger, self).__init__(trunk)
        self.left_operator = cairo.OPERATOR_ADD
        self.right_operator = cairo.OPERATOR_ADD
        self.left_alphablending = 1.0
        self.right_alphablending = 1.0

    def __eq__(self, other):
        """Randomize the mergers components."""
        equal = isinstance(other, CombineMerger) \
                ...
                and self.right_alphablending == other.right_alphablending
        return equal
    ...
    def swap_places(self):
        ...
        self.left_operator, self.right_operator = \
           model_random.swap_parameters(self.left_operator, self.right_operator)
        self.left_alphablending, self.right_alphablending = \
           model_random.swap_parameters(self.left_alphablending, self.right_alphablending)

    def crossingover(self, other):
        """
        pre: isinstance(self, CombineMerger)
        pre: isinstance(other, CombineMerger)
        # check for distinct references, needs to copy content, not references
        post: __return__ is not self
        """
        new_one = CombineMerger(self.get_trunk())
        crossing = model_random.crossing_sequence(4)
        new_one.left_operator = other.left_operator if crossing[0] \
                                                   else self.left_operator
        new_one.right_operator = other.right_operator if crossing[1] \
                                                   else self.right_operator
        new_one.left_alphablending = other.left_alphablending if crossing[2] \
                                                   else self.left_alphablending
        new_one.right_alphablending = other.right_alphablending if crossing[3] \
                                                   else self.right_alphablending
        return new_one
...
#        ka_debug.matrix_r(ctx.get_matrix())
    ...
    def copy(self):
        """A copy constructor.
        post: isinstance(__return__, CombineMerger)
        # check for distinct references, needs to copy content, not references
        post: __return__ is not self
        """
        new_one = CombineMerger(self.get_trunk())
        new_one.left_operator = self.left_operator
        new_one.right_operator = self.right_operator
        new_one.left_alphablending = self.left_alphablending
        new_one.right_alphablending = self.right_alphablending
        return new_one
